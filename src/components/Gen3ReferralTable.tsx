...
const Gen3ReferralTable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Generation 2: referrals by Gen 1
    const gen1Ids = gen1.map(user => user.userId);
    const gen2 = users.filter(user => gen1Ids.includes(user.referrerId));
    setGen2Users(gen2);

    // Generation 3: referrals by Gen 2
    const gen2Ids = gen2.map(user => user.userId);
    const gen3 = users.filter(user => gen2Ids.includes(user.referrerId));
    setGen3Users(gen3);
  ...
  return (
    <div className="p-1 w-full">        
      ...
      {(gen1Users.length > 0 || gen2Users.length > 0 || gen3Users.length > 0) && (
        <table className="table-auto w-full mt-4 border-collapse border border-gray-400">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td className="border border-gray-400 px-4 py-2 text-center">{gen3Users.length}</td>
            </tr>
          </tbody>
        </table>
      )}
...
export default Gen3ReferralTable;
